...
class AsynapRous:
    ...
    def prepare_auth(self, credentials=None, realm="AsynapRous"):
        """
        Configure default Basic Auth credentials for protected routes.

        :param credentials: Dictionary mapping username to password.
        :param realm: Authentication realm sent in WWW-Authenticate.
        """
        self.auth_credentials = credentials
        self.auth_realm = realm

    def route(self, path, methods=['GET'], auth=False, credentials=None, realm=None):
        """
        ...
        """
        def decorator(func):
            func._auth_required = auth
            func._auth_credentials = credentials or self.auth_credentials
            func._auth_realm = realm or self.auth_realm
