...
python3 start_sampleapp.py --role tracker --server-port 8000 > tracker.log 2>&1 &
TRACKER_PID=$!
...
python3 start_sampleapp.py --role peer --username alice --server-port 9001 > alice.log 2>&1 &
ALICE_PID=$!

# 3. Start Peer Bob
echo "[3/4] Starting Peer Bob on port 9002..."
python3 start_sampleapp.py --role peer --username bob --server-port 9002 > bob.log 2>&1 &
BOB_PID=$!
